...
class POS_MEMM:
    ...
    def train(self, data_path, regularization = 0.1, mode = 'base', spelling_threshold = 10):
        self.regularization = regularization
        self.mode = mode
        
        (V, T_with_start, data, data_tag) = self.data_preprocessing(data_path, 'train')
        ...
        self.V = V
        self.T = T
        self.T_with_start = T_with_start
        self.T_size = len(T_with_start)
        self.V_size = len(V)
        self.T_dict = T_dict
        self.V_dict = V_dict
        self.T_with_start_dict = T_with_start_dict
        ...
        print('FStart training...')
        t0 = time.time()
        opt_params = fmin_l_bfgs_b(self.loss, x0=self.weights, fprime=self.loss_grads)
    ...
    def calc_all_possible_tags_probabilities(self, xi, t1, t2, w):
        """
    ...
    def loss(self, w):
        ...
        for h, sentence in enumerate(self.data):
            ...
            for i, word in enumerate(sentence[:-1]):
                ...
                for j, tag in enumerate(T):
                    exp_term[j] = np.sum(w[self.get_features(word, [tag_sentence[i-2], tag_sentence[i-1], tag], mode)])
    ...
    def data_preprocessing(self, data_path, mode):
    ...
    def get_feature_size(self):
        ...
        return np.sum(size_dict.values())
                ...
                if k > 1 and not prob_mat[V_COMP_dict[sentence[k]],0,0,0].any():
                    for u in self.T: # for each t-1 possible tag
                        for t in self.T: # for each t-2 possible tag:
                            prob_mat[V_COMP_dict[sentence[k]],:, self.T_dict[u], self.T_dict[t]] = self.calc_all_possible_tags_probabilities(sentence[k], u, t, self.weights)
                ...
                for current_tag in self.T: # for each t possible tag
                    ...
                    if k == 0:
                        ...
                        for u in T: # for each t-1 possible tag
                            pi_matrix[k, self.T_dict[u], :] = pi_matrix[k - 1, 0, self.T_dict[u]] * self.calc_all_possible_tags_probabilities(sentence[k], u, '/*', self.weights)
                        break
                    else:
                        for u in T: # for each t-1 possible tag
                            # calculate pi value, and check if it exeeds the current max:
                            pi_values = pi_matrix[k-1, :, self.T_dict[u]] * prob_mat[V_COMP[sentence[k]],self.T_dict[current_tag], self.T_dict[u], :]
    ...
    def get_features(self, word, tags):
        ...
        try:
            ...
            tmp = 0 #must do something in exept
        F100_len = self.V_size * self.T_size
    
        # trigram feature - 1 if <t(i-2),t(is),t(i)> = <t1,t2,t3>
        F103 = self.T_with_start_dict[tags[2]] * (self.T_size ** 2) + self.T_with_start_dict[tags[1]] * self.T_size + self.T_with_start_dict[tags[0]]
        ...
        if self.mode=='complex': #complex featurs:
    ...
    def test(self, test_data_path, end = -1 , start = 0):
        
        (_, _, test, test_tag) = self.data_preprocessing(test_data_path, 'test')
           
        corpus = test[start:end]
        corpus_tag = test_tag[start:end]
        
        # run Viterbi algorithm
        (all_tagged_sentence, all_sentence_tags) = self.predict(corpus)
