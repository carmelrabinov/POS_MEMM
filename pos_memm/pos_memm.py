        ...
        for pred, real in zip(tag_line, tag_real[i]):
            if pred != real:
                ...
                if j < len(tag_line)-1:
                    ...
                    next_word = '/stop'
                    next_tag  = '/stop'
                res.append([pred, prev_tag, real, next_tag, prev_word, data_real[i][j], next_word, j, i, 1-int(data_real[i][j] in V)])
            j += 1

    df = pd.DataFrame(res, columns=['pred', 'prev tag', 'real', 'next tag', 'previous word', 'word', 'next word', 'word index', 'sentence index', 'unknown word'])
...
#                                            self.get_features(word, [tag_sen[i - 2], tag_sen[i - 1], tag])
    ...
    def train(self, data_path, regularization=0.1, mode='base', spelling_threshold=8,
              word_count_threshold=3, verbosity=0, log_path=None):
    ...
    def get_feature_size(self):
        ...
        if self.mode == 'complex':
            size_dict['F101_1'] = self.T_size*len(self.suffix_1) # all posible tags for each word in importnat suffix list
            ...
            size_dict['F102_1'] = self.T_size*len(self.prefix_1) # all posible tags for each word in importnat prefix list
            ...
            size_dict['G7_2'] = self.T_size**2  # is the word unknown (train: less then threshold), not a number and t(i-1,i)
            size_dict['G7_3'] = self.T_size**3  # is the word unknown (train: less then threshold), not a number and t(i-2,i-1,i)
    ...
    def get_features(self, word, tags, is_first=False):
        ...
        if self.mode == 'complex':
            ...
            # F101: suffix of length  1/2/3/4 which is in suffix lists && tag <t(i)>
            if word_len > 1 and word[-1:] in self.suffix_1.keys():
                F101_1 = self.suffix_1[word[-1:]] * self.T_size + self.T_with_start_dict[tags[2]]
                features.append(F101_1 + F104_len)
            F101_1_len = F104_len + self.T_size * len(self.suffix_1)

            if word_len >= 2 and word[-2:] in self.suffix_2.keys():
                F101_2 = self.suffix_2[word[-2:]] * self.T_size + self.T_with_start_dict[tags[2]]
                features.append(F101_2 + F101_1_len)
            ...
            if word_len > 1 and word[:1] in self.prefix_1.keys():
                F102_1 = self.prefix_1[word[:1]] * self.T_size + self.T_with_start_dict[tags[2]]
                features.append(F102_1 + F101_len)
            F102_1_len = F101_len + self.T_size * len(self.prefix_1)
            ...
            F102_2_len = F102_1_len + self.T_size * len(self.prefix_2)
            ...
            if (self.train_or_predict_mode == 'predict' and word not in self.V and not is_number) or \
                    (self.train_or_predict_mode == 'train' and self.V_count[word] <= self.word_count_threshold and not is_number):
                ...
                # G7_3 : tag trigram
                G7_3 = self.T_with_start_dict[tags[2]] * (self.T_size ** 2) + self.T_with_start_dict[tags[1]] * self.T_size + \
                                                                    self.T_with_start_dict[tags[0]]
    ...
    def predict(self, corpus, verbosity=0, log_path=None):
        ...
        """
        self.train_or_predict_mode = 'predict'
        self.verbosity = verbosity
    ...
    def predict_parallel(self, corpus, verbosity=0, log_path=None):
        ...
        pool = Pool(processes=5, maxtasksperchild=3)
        ...
        print('predict_parallel in time: {} min'.format(prediction_time))
        return all_tagged_sentence, all_sentence_tags
    ...
    def build_predict_prob_matrix(self, corpus):
        ...
        res = pool.map(self.build_word_prob_matrix, V_COMP[0:20])
        pool.close()
        pool.join()
